...
- **并发是唯一被证明有效的吞吐手段**（与单流 146 t/s 相比）：np=16 短 prompt 聚合 627.8 t/s（4.30×）、np=4 长 prompt 场景 317–327 t/s。注意并发测试**必须先做并发预热**（首轮含 CUDA 图对目标 batch size 的捕获，实测首轮 225 → 稳态 327 t/s）。
