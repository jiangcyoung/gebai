...
> - **分片边界与滚动语义**：切分点由 `history-chunk.ts` 的 `historySplitIndex`（纯函数，可单测）计算，**向前扩展到执行过程容器（`subSessionId`）的分组边界**——同一次 `subsession_run` 的过程消息必须整组落在同一段，否则会被拆成两个折叠容器。前插补齐**等量补偿 `scrollTop`**（`scrollHeight` 增量），阅读中的视口内容原地不动。
...
- **行内 blame（`files/blame.ts` 文本规则 + `files/editor.ts` 的 `setBlame` 渲染）：两态两个按钮、互相独立**——① **侧边列**（`gutter`）：编辑器左侧独立一列（`.fw-blame-gutter`，在 Monaco 容器**之外**：flex 兄弟，右侧 1px 弱线分隔），逐行显示「作者 · 时间」；行位置用 `ed.getScrolledVisiblePosition()` 随滚动/折行按帧重算（只渲染可见行，行节点池复用），未提交行转警告色；② **光标行行尾**（`inline`）：`after` 注入到光标所在行行尾，样式更弱（淡色 + 斜体 + 半透明），光标移动只更新这一个装饰集合。两者共用一份数据（`/git/blame`），可以只开一个或都开。**只有行尾态的开关记在浏览器本地**（`files/blame-prefs.ts` → `gebai.ui.blameInline`）：它是一条跟随光标的淡色注释，常开不妨碍阅读；侧边列占一行宽度，每次打开文件按关闭处理。两者都是装饰/注入文本，**不进模型**：复制、保存、撤销拿到的仍是原文。三个必须踩准的点：① 装饰集合用**编辑器实例**的 `editor.createDecorationsCollection()`——`monaco.editor.createDecorationsCollection` 不是静态 API，取值得到 `undefined` 后静默降级为「什么都不画」（按钮照常高亮、不报错）；② 注入文本挂在**空 range** 上时必须显式 `showIfCollapsed: true`（Monaco 取注入文本会按 `showIfCollapsed || !range.isEmpty()` 过滤，否则整条装饰被静默丢弃）；③ 时间取**相对值**（`ui.ts` 的 `timeAgo`）：blame 关心的是「多久前有人动过这里」，绝对时间戳要占 16 字符；作者名截 14 字符。编辑态下两态都不可用（行号会随编辑漂移，注释会指到别的行；切回查看态时行尾态按本地偏好恢复）。三个必须踩准的点：① 装饰集合用**编辑器实例**的 `editor.createDecorationsCollection()`——`monaco.editor.createDecorationsCollection` 不是静态 API，取值得到 `undefined` 后静默降级为「什么都不画」（按钮照常高亮、不报错）；② 注入文本挂在**空 range** 上时必须显式 `showIfCollapsed: true`（Monaco 取注入文本会按 `showIfCollapsed || !range.isEmpty()` 过滤，否则整条装饰被静默丢弃）；③ 时间取**相对值**（`ui.ts` 的 `timeAgo`）：blame 关心的是「多久前有人动过这里」，绝对时间戳要占 16 字符；作者名截 14 字符。编辑态下不开放（行号会随编辑漂移，注释会指到别的行）。
...
- **安全与审计**：`GEBAI_FS_ENABLED`（总开关，false → 页面与 fs/git 端点全 404）/`GEBAI_FS_WRITE`（工作台写开关）/`GEBAI_GIT_WRITE`/`GEBAI_GIT_REMOTE`；写操作统一审计（`GEBAI_FS_AUDIT` → `{GEBAI_HOME}/audit-fs.jsonl`）；删除入回收站（可恢复/彻底清除）；上限 `GEBAI_FS_MAX_READ|WRITE|UPLOAD|ZIP`，以及差异侧的 `GEBAI_FS_MAX_DIFF` / `GEBAI_GIT_MAX_FILE`（见下文「体量与失败」）。
...
- **界面结构（无标题栏三区）**：整体对齐 IDEA **新 UI**——去掉传统标题栏/菜单栏，入口交给**左侧活动栏**与各面板自身：
  - 活动栏分两组（中间以细分隔）：**上组 = 视图切换**（资源管理器 / 搜索 / 变更，三视图**互斥**——同一时刻只看一件事：要么在找文件、要么在看改动）；**下组 = 工具窗与全局入口**（Git 工具窗开关 / 回收站 / **更多**）；**嵌入（分屏）时最下方多一个「关闭分屏」**（图标与「更多」里那一项同款 `collapseRight`——面板停在窗口右列，箭头指出向；走既有的 `requestCloseSplit()`）——面板内最直接的那个出口，独立标签页不渲染。
  ...
  - **标签栏右侧只留两个高频动作，其余进轮盘**（与标题栏轮盘同一套交互，共用 `wheel-core.ts` + `css/wheel.css`；入口 hover 展开、键盘 Enter/空格/↓ 同样展开，Esc/外点/resize 收起）：常驻「编辑/查看」与「保存」——它们每一步编码都要碰；行内 blame 两态俩按钮（行尾 / 侧边列）/ 源码⇄渲染预览 / 重新加载 / 下载 / 文件历史 / 复制路径 按频率分到内弧（看这个文件）与外弧（把它带出去）。两弧分别是“偶尔用一次”的动作，铺成一排会把编辑区右上角变成按钮墙、也把高频动作淹没。两个常驻按钮与轮盘入口均仅图标、随活动标签类型变化；Git 动作在各自面板里。
...
- **两种打开方式**：标题栏**左侧**（会话列表按钮右边）的入口主按钮 = **分屏打开**（iframe 嵌工作台，缺省**停靠在左**、会话区在右，默认五五开、分界可拖双击复位；图标为**外框+中间竖线**——一眼就是"一窗分两栏"）；它 **hover 时右方**弹出的副按钮 = **新标签打开**（整个工作台页面，图标即原空心文件夹 + `<>`）。消息流里的文件产物（文件卡 / 链接 / 原文件弹窗）也走同一入口，自动带上所属会话。
  ...
  - 关闭分屏**不是标题栏上的 ✕**，而是面板内与全局几条路径：工作台活动栏最下方的「关闭分屏」、工作台「更多」菜单的「关闭分屏」、Esc（两侧都能触发）、Ctrl+Shift+E，以及点标题栏的「会话列表」（分屏时它本就被折叠，点它＝把列表拿回来，顺带关分屏）。活动栏与菜单里那个「关闭分屏」的箭头**指出向、随停靠侧翻转**（面板在左 → `collapseLeft`，在右 → `collapseRight`）——它与新图标 `swap`（左右互换）同住在 `files/ui.ts`，停靠侧由宿主经 `gebai:files-split-side` 推给工作台。
  ...
  - **面板是纯容器，没有标题栏**：只有分界拖条 + iframe。那条 30px 横条要花掉整个编辑区的垂直空间，而它的按钮各有去处——**关闭＝也不在标题栏**（走面板内与全局路径：活动栏最下方的「关闭分屏」/工作台「更多」菜单、Esc、Ctrl+Shift+E、点「会话列表」）；打开＝入口的两个按钮；重新加载／在新标签打开／分屏停靠改到左(右)侧／关闭分屏＝工作台自己的「更多」菜单（页面级动作归页面自己）。副按钮保持原状：图标不随分屏态变形、也不常驻（只在浮空时出现），它永远只表达"新标签打开"。
  - **Esc 的作用域**：父文档监听 Esc 关分屏（焦点在主界面时有效）；iframe 内的按键父文档收不到，子页另转发一次——必须**捕获阶段 + 事前检查有无浮层**：菜单/对话框的 Esc 处理器在冒泡阶段且不 `stopPropagation`，等它们关完菜单再判断，浮层已经不在 DOM 里，分屏会被连带关掉。
...
- **资源管理器头部（单行 + 按需过滤）**：头部只有一行——根选择按钮占主宽（点击弹根清单），右侧一排小图标（新建文件 / 新建文件夹 / 刷新 / 更多）。**不再有常驻的过滤输入框与面包屑**：目录过滤改为按需展开的输入行（`Ctrl+Alt+F` 或「更多」菜单入口，`Esc` 收起并清空），排序 / 显示隐藏文件 / 上传 / 折叠全部收进「更多」菜单（当前态用 `✓ ` 前缀标记）——高频动作留图标、低频动作进菜单，省下的三行纵向空间全给树。面包屑移除的理由：标签页与树的选中行已分别表达「在看什么文件」与「在哪个目录」，再摆一行面包屑是重复信息。
...
- **底部工具窗：Git 与终端同槽互斥**：底部工具窗（`.fw-git-dock`）承载两个视图——Git（分支 | 日志 | 提交内容）与终端，共用同一停靠位、同一高度变量（`--git-dock-h`）与同一拖拽条；两者**实例都保留**，切换只切显隐类（`.fw-dock-hidden`），因此 Git 的滚动位置与终端的滚动缓冲/会话都不会丢。可见性与当前视图持久化（`gebai.ui.dockVisible` / `gebai.ui.dockView`，无记忆时按窗口宽度、<1180px 默认收起）；入口＝活动栏终端/Git 按钮、`Ctrl+Alt+T`（终端）/`Ctrl+Alt+G`（Git）、状态栏分支项。
- **终端面板（真 PTY + xterm）**：分两层——**PTY 通道**（Windows ConPTY）与**降级通道**（管道式持久 shell）。
  ...
  - **中断语义（`term.interrupt`）**：终止进程树并以**原 cwd / 尺寸**重建 shell（会话 id、回放缓冲、订阅全保留）。为何不是直写 `\x03`：ConPTY 下向输入管道写 ETX 不会生成 CTRL_C_EVENT（实测 `ping -t` / `timeout /t` 均不停），所以「Ctrl+C 确实能停下当前命令」只能靠重建实现，代价是 shell 内部状态（`set` 的变量、`cd` 后的目录）丢失。
...
- **已知边界**：Office 预览非像素级一致（兜底下载打开）；超大文件只读片段并提示下载；无内置 hex 编辑；冲突合并为**文本级三窗格**（不做语义/结构化合并，二进制冲突只能选一侧或外部处理）；未做 Git 子模块详情、LFS；终端 `Ctrl+C` 的语义是「终止当前命令并以原 cwd 重建 shell」（ConPTY 不接受 ETX 字节，见上），因此 shell 内部状态（`set` 变量、`cd` 后的位置）在中断后回到会话创建时的 cwd；PTY 驱动依赖 Windows 自带的 .NET Framework 编译器，非 Windows 或缺失该组件时终端自动降级为管道式实现（无 TTY 语义，全屏交互式程序不可用）；终端面板的「跟随当前根」只在切换根时补一条 `cd`，不跟踪 shell 内的 `cd`（无法从无提示符的字节流里可靠地读出 cwd）。
