...
- **界面结构（无标题栏三区）**：整体对齐 IDEA **新 UI**——去掉传统标题栏/菜单栏，入口交给**左侧活动栏**与各面板自身：
  ...
  - **Git 工具窗不设常驻标题栏**：那一行里每一件都有别的载体——面板身份＝活动栏高亮（「源代码管理」四字本身不是信息）、当前分支＝状态栏的分支项（以及分支列表的 ✓）、「比较」＝活动栏「更多」与 `Ctrl+Shift+D`、刷新＝各栏自己的刷新与 F5、关闭＝活动栏按钮 / `Ctrl+Alt+G`。仅有两件事**别处说不了**：多步操作进行中（merge/rebase 与冲突数）与在途写操作（fetch/pull/push 耗时以秒计）。因此这一行**空闲时整行隐藏、零高度**，只在这两件事发生时亮成一条状态带（上下两条线此时才出现，把这条带框出来）。
