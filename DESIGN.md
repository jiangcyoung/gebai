...
| 粘底阈值 | 64px | 距底 ≤ 该值视为在底部（`scroll-intent.ts` 的 `threshold` 与按钮/几何判定共用） |
...
| hold 距离 | 1.5 屏 | 距底在该范围内且不在追最新 → `hold`，超出 → `reading`（按钮显隐相同） |
