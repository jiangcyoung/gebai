...
- **键盘快捷键（唯一来源 `packages/web/src/keymap.ts`）**：全站键位收进一张声明表（主界面 `keymap-main.ts`、文件工作台 `files/keymap-wb.ts`、工作台动作绑定 `files/main.ts`），分发器统一负责焦点守卫、监听阶段与拦截；**键位取常用键，能接管浏览器的就接管**——`Ctrl+S` 保存、`Ctrl+P` 快速打开、`Ctrl+F` 目录过滤、`Ctrl+K` 更多菜单、`F5` 刷新、`Ctrl+Shift+E/F/G` 切面板、终端 `Ctrl+Shift+C/V` 复制粘贴。依据是 Chromium 的两级处理：`BrowserView::PreHandleKeyboardEvent()` 对**非保留**命令返回 `NOT_HANDLED_IS_SHORTCUT`（按键先到页面，`preventDefault` 即接管），只有 `BrowserCommandController::IsReservedCommandOrKey()` 那几条**保留命令**（`Ctrl+N/T/W`、`Ctrl+Shift+N/T/W`、`Ctrl+Tab`、`Ctrl+PageUp/Down`、`Ctrl+Shift+Q`；`TYPE_APP`/PWA/桌面形态下一条都不保留）是页面真拿不到的。判据做成可执行的 `browserConflict()`，返回值 `free | override | reserved` 三档，表里必须为后两档显式声明 `browser: "override" | "reserved"`（接管要真拦截、保留要写 `note` 说明后果），测试遍历全表断言声明完整。守卫集中在分发器：终端面板内全局键让位（shell 的 readline 键回归）、输入框内只有编辑器查找类（`Ctrl+F`）让位、输入法组合态与长按重复不触发；弹窗/菜单/查看器以下拉式作用域入栈，**`Esc` 只关最上层**（取代此前 11 个文档级监听各自广播）。帮助 UI 与文档由 `helpGroups()` 从表生成（标题栏轮盘「快捷键」、工作台「更多 → 快捷键」，接管浏览器的键位带「接管 xx」标注），全表见 `docs/keyboard-shortcuts.md`。
...
- **键盘快捷键**：与主界面共用同一套机制（`../keymap.ts` 分发器 + `files/keymap-wb.ts` 建表）——`Ctrl+S`（保存）、`Ctrl+P`（快速打开）、`Ctrl+W`（关标签）、`Ctrl+E`（查看↔编辑）、`Ctrl+F`（目录过滤）、`Ctrl+B`（左栏）、`Ctrl+Shift+E/F/G`（资源管理器/搜索/变更）、`Ctrl+K`（更多菜单）、`Ctrl+Shift+D`（比较）、``Ctrl+` ``（终端）、`F5`（刷新）、`Alt+G`（Git 工具窗）、`F7`/`Shift+F7`（差异块）、`F8`/`Shift+F8`（变更文件 / 合并冲突）、`Alt+M`（标记为解决）、`F2`、`Alt+Z`、`Esc`；终端面板内 `Ctrl+Shift+C/V`（复制/粘贴）、`Ctrl+F`（搜索）、`Ctrl+=/-/0`（字号）、`Ctrl+C`（中断）、`Ctrl+L`（清屏，降级终端）。接管浏览器默认的键位一律走**捕获阶段**（抢在 Monaco / xterm 之前，也避开 Monaco 把 `Ctrl+K` 当多键前缀、`F7` 当 diffReview）；焦点在终端面板内时全局键一律让位，`Ctrl+W/P/E/K/B` 交还 shell 的 readline。`Ctrl+W` 是 Chromium 保留命令（页面收不到），只在桌面/app 形态生效，浏览器窗口里的误关由工作台 `beforeunload`（有未保存改动时确认）兜底。
...
- **两种打开方式**：标题栏**左侧**（会话列表按钮右边）的入口主按钮 = **分屏打开**（iframe 嵌工作台，缺省**停靠在左**、会话区在右，默认五五开、分界可拖双击复位；图标为**外框+中间竖线**——一眼就是"一窗分两栏"）；它 **hover 时右方**弹出的副按钮 = **新标签打开**（整个工作台页面，图标即原空心文件夹 + `<>`）。消息流里的文件产物（文件卡 / 链接 / 原文件弹窗）也走同一入口，自动带上所属会话。
  ...
  - 关闭分屏**不是标题栏上的 ✕**，而是面板内与全局几条路径：工作台活动栏最下方的「关闭分屏」、工作台「更多」菜单的「关闭分屏」、Ctrl+\，以及点标题栏的「会话列表」（分屏时它本就被折叠，点它＝把列表拿回来，顺带关分屏）。活动栏与菜单里那个「关闭分屏」的箭头**指出向、随停靠侧翻转**（面板在左 → `collapseLeft`，在右 → `collapseRight`）——它与新图标 `swap`（左右互换）同住在 `files/ui.ts`，停靠侧由宿主经 `gebai:files-split-side` 推给工作台。
  ...
  - **面板是纯容器，没有标题栏**：只有分界拖条 + iframe。那条 30px 横条要花掉整个编辑区的垂直空间，而它的按钮各有去处——**关闭＝也不在标题栏**（走面板内与全局路径：活动栏最下方的「关闭分屏」/工作台「更多」菜单、Ctrl+\、点「会话列表」）；打开＝入口的两个按钮；重新加载／在新标签打开／分屏停靠改到左(右)侧／关闭分屏＝工作台自己的「更多」菜单（页面级动作归页面自己）。副按钮保持原状：图标不随分屏态变形、也不常驻（只在浮空时出现），它永远只表达"新标签打开"。
