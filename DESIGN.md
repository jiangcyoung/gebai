...
- **界面结构（无标题栏三区）**：整体对齐 IDEA **新 UI**——去掉传统标题栏/菜单栏，入口交给**左侧活动栏**与各面板自身：
  ...
  - **标签栏右侧只留高频动作，其余进轮盘**（与标题栏轮盘同一套交互，共用 `wheel-core.ts` + `css/wheel.css`；入口 hover 展开、键盘 Enter/空格/↓ 同样展开，Esc/外点/resize 收起；**弧位按项数与按钮尺寸自适应**——从起始角向左侧撑角度，不够再二分加大半径，相邻按钮方块的间距不小于「边长 + 间隙」，最上方按钮不越过入口那一行），常驻「编辑/查看」，以及**可渲染文件（mermaid / plantuml / d2 / echarts）多一个「渲染/原文」切换**（它决定看到的是图还是文本，比其它动作都要紧；图标随之在“切换到渲染预览 / 切换到源码”间切换并标 active）；轮盘**内弧 = 历史相关三个**（文件历史 / 行尾 blame / 侧边 blame 列——都回答“这行、这文件是什么时候、谁改的”，是一类动作），**外弧 = 文件本身的动作**（保存 / 重载 / 下载 / 复制路径；保存带脏标记高亮与禁用态）。按钮区**不画分界线**（它紧贴标签栏右端，多一条线只是把两个相邻区域切成两栏），与标签栏底色一致。两弧分别是“偶尔用一次”的动作，铺成一排会把编辑区右上角变成按钮墙、也把高频动作淹没。两个常驻按钮与轮盘入口均仅图标、随活动标签类型变化；Git 动作在各自面板里。
