...
- **原生的热点与优化原则**：扫描阶段已到 366 MB/s（与 Go 重写同量级），剩余耗时在**逐事件的字符串分配**与 **HashMap 重查**——优化手段是把字段提取改为**借用切片**（`Cow` / `&[u8]`，仅在新分组插入时才 `to_owned`）、查表用借用键、复用键拼接缓冲。实现内保留基准探针 `TORCH_BENCH_PROBE=noop|parse`（只扫描 / 叠加字段提取）——性能回归靠它定位到阶段，不靠盲猜。
