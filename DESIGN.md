...
- 工具定义可声明 `card: { titleParams?, args?, codeField?, codeLang?, file? }`，经 `session.tool.get` 随 ToolInfo 下发，前端按声明渲染工具调用卡片，**不在前端硬编码工具名**：
  ...
  - **标题参数不在参数区重复展示**（titleParams 已入标题的键从参数区省略，如 `read` 参数区只显示 offset/limit；全部参数入标题时无参数区）；**超长参数区折叠只发生在完成态**（>800 字符收敛为「查看参数（N 字符）」折叠块，默认收起，点击展开，与输出折叠同款交互）——**执行/审批等待期完整直显不折叠**（实时调用卡 `🛠` 态带全量参数，执行与审批时可查看全貌），结果到达（`✓` 完成态，appendToolResult）时收敛为折叠块，与历史回放同构；subsession_run/subsession_run 专用参数块不参与折叠
...
- **界面结构（无标题栏三区）**：整体对齐 IDEA **新 UI**——去掉传统标题栏/菜单栏，入口交给**左侧活动栏**与各面板自身：
  ...
  - 「更多」承接原菜单栏的杂项（新建文件·文件夹 / 上传 / 比较 / 刷新根清单 / 快捷键一览 / 服务端开关 / 全屏 / 返回主界面），`Ctrl+K` 直开；文件类动作同时在树右键菜单里有（两处入口，不是唯一路径）。**没有面包屑与单独的工具条行**：路径信息由标签标题 + 资源管理器表达，动作按钮（编辑/保存/预览/下载/历史/重载/复制路径）全部收进**标签栏右侧、仅图标**，随活动标签类型变化；Git 动作在各自面板里。
