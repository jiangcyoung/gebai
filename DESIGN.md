...
- **渲染档与实测**：NVENC（需 Remotion ≥ 4.0.484）/VideoToolbox 硬件编码（用 `videoBitrate`，硬件档不支持 crf）、Linux+NVIDIA 才用 `chrome-for-testing`、**非 WebGL 内容不传 gl**（默认后端更优且 angle 有内存泄漏风险）、并发**自动档取有效核数的一半**（与 Remotion 官方默认 `min(8, cores/2)` 同口径；全片实测 c=2 与 c=4 **无差异**（835 帧 1080p，各 4 次：均值 41.4s vs 41.1s，在噪声内）——取半档是为少占内存与对齐官方口径，**不是为提速**：帧渲染受浏览器侧固定开销主导，加并发既不能突破 4 核配额也不会更快；注意并发结论**不能用短帧段样本得出**：同一机器上 60 帧样本曾显示 c=2 快 8%，全片复验证伪）、被拒时按报错上限**自愈重试一次**、`bench` 用 `hardwareAcceleration=required` 强制探针实测并写调优缓存（默认并发候选＝有效核数与其一半，未显式传也**不空跑**；缓存按库根隔离并在结果里给出文件路径）
...
- **输出尺寸与画质档（`output.ts`）**：视频尺寸用 `height=<目标高>`（按合成长宽比换算）或 `scale=<比例>`，二选一；换算结果必须**整数且偶数**（h264 要求）——不合法时在工具入口即报错并**按当前合成列出可用档位**（`feasibleSizes`），不把问题留到编码阶段（自由浮点 scale 写 0.667 会得到 1281×720 而停在 ffmpeg 报错里，与「尺寸没算对」离得很远）。`quality=draft` 为确认动效与节奏的快速档：**只降视频**（半分辨率 + `ultrafast` 编码 + 帧图质量 70），**静帧不降**（抽帧是判读细节用的）；`x264_preset` 可调编码速度档（默认 Remotion 内置 medium）。依据（4 核 · 无 GPU · 软件编码 · 830 帧 27.7s 片 · 热态实测）：全片 1080p final **42s**、1080p+ultrafast 38s、540p draft **35s**、360p draft 32s（复测稳定）；节级草稿 120/230 帧分别 5s / 10s；单张抽帧约 0.4s。即**降分辨率收益有限**（540p 只快 17%——每帧的浏览器渲染/合成/截图回读是与像素量弱相关的固定成本），草稿档必须**做小**（节级 / 抽帧）而不是整片低清：全片草稿 35s + 正片 42s = 77s，比直接出正片（42s）贵 83%。另：**开头几十帧的耗时不可外推到整片**（元素密集的开场镜头与首个作业的固定开销都会高估——以 60 帧样本外推约高估一倍）
...
- **自动换行（`Alt+Z` / 动作轮盘）**：换行是**用户级偏好**（`gebai.ui.wordWrap`，默认关闭——代码的横向滚动是默认阅读方式），主编辑器、差异视图（两侧一起）、合并/暂存窗格与降级编辑器（textarea 走 CSS 类）都由 `files/editor.ts` 的**模块级开关 + 活动实例注册**统一施加：切换点有轮盘项与 `Alt+Z` 两处，而实例可能同时有十几个（每个标签一个，合并视图一次三个），把「记住开关」与「应用到全部」收在一处才保证一致（实例 dispose 自行注销）。`Alt+Z` 与 `Ctrl+Alt+↑↓` 同一处置：**捕获阶段**拦下——Monaco 自己绑了这个键（`editor.action.toggleWordWrap`），但它只改实例选项、不动偏好（刷新即回退）也不更新按钮态，而且编辑器获焦时事件到不了冒泡阶段；**表单输入框里不拦**（Mac 上 Option+Z 是输入 Ω 的手势，在提交信息框/搜索框里打字时被快捷键抢走才是真的碍事），但 Monaco 与降级编辑器内部的隐藏输入区也是 textarea，靠「是否在编辑器容器内」区分。
