...
- **浏览器缓存位置**按 Remotion 自身规则（进程 cwd 向上最近 `package.json` 的 `node_modules/.remotion`），查找为**多候选根**：Remotion 规则根优先，其次工程目录与共享运行时（Remotion 自身不查这两处，但本机已有即可直接继承——渲染总是把命中的可执行文件显式交给原生库），由 setup/status 如实报告目录与体积
