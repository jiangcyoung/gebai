...
- **行内 blame（`files/editor.ts` 的 `setBlame`）**：每行行首注入「作者 · 时间」注释（未提交的行标「未提交」并铺一层警告色行底），悬浮给完整提交信息（哈希 / 作者 / 绝对时间 / 摘要）。三个必须踩准的点：① 装饰集合用**编辑器实例**的 `editor.createDecorationsCollection()`——`monaco.editor.createDecorationsCollection` 不是静态 API，取值得到 `undefined` 后静默降级为「什么都不画」（按钮照常高亮、不报错）；② 注入文本挂在**空 range** 上，而 Monaco 取注入文本时会过滤掉空 range 装饰（`showIfCollapsed || !range.isEmpty()`），必须显式 `showIfCollapsed: true`，否则整条装饰被静默丢弃；③ 注释宽度**固定 22ch**（`inline-block` + 省略号）——各行作者名与相对时间长短不一，不固定宽度则每行代码的起始列都不一样，通篇看下来像被锯齿切过。时间取相对值（`ui.ts` 的 `timeAgo`）：blame 关心的是「多久前有人动过这里」，绝对时间戳要占 16 字符。编辑态下不开放（行号会随编辑漂移，注释会指到别的行）。
...
- **界面结构（无标题栏三区）**：整体对齐 IDEA **新 UI**——去掉传统标题栏/菜单栏，入口交给**左侧活动栏**与各面板自身：
  ...
  - **标签栏右侧只留两个高频动作，其余进轮盘**（与标题栏轮盘同一套交互，共用 `wheel-core.ts` + `css/wheel.css`；入口 hover 展开、键盘 Enter/空格/↓ 同样展开，Esc/外点/resize 收起）：常驻「编辑/查看」与「保存」——它们每一步编码都要碰；行内 blame / 源码⇄渲染预览 / 重新加载 / 下载 / 文件历史 / 复制路径 按频率分到内弧（看这个文件）与外弧（把它带出去）。两弧分别是“偶尔用一次”的动作，铺成一排会把编辑区右上角变成按钮墙、也把高频动作淹没。两个常驻按钮与轮盘入口均仅图标、随活动标签类型变化；Git 动作在各自面板里。
