...
### 6.12 结构性根因（已定位，属引擎内部）

高详细度日志给出了直接证据——**每次 decode 的计算图被切成两段**：
...
即：主计算在 CUDA0，而**嵌入表驻 CPU + logits 输出缓冲在主机内存**导致图被切为两段，
每步都要跨后端同步与拷贝。这是 llama.cpp 对「主机内存张量 + 循环状态」的调度方式决定的
**结构性开销，不是参数问题**。
...
| 尝试 | 结果 |
|---|---|
| `-ot token_embd.weight=CUDA0`（把嵌入表推到 GPU） | 140.10 vs 基线 138.81 —— 噪声内，无改善 |
| `--no-host` | 参数被拒（实现不完整） |
| 所有线程/批/FA/KV/CUDA 环境变量旋钮 | 均落在噪声区间（见 6.6） |
| 换平台（WSL/Linux 官方构建） | 137.46 vs 140.98 —— 一致，非平台问题 |

**唯一有效的方向是批处理**（同步与拷贝是按步计费、与批量无关）：
`-np 8` 聚合 2.67×；统一批处理理论 4.1×。

**若要进一步提速（需改引擎代码）**，措施必须是“减少每步的同步/拷贝次数”：
合并两段图（或把嵌入/logits 全部设备化）、用后端采样消除 logits 回传（`-bs` 已测到 +5%）、
将循环状态缓冲常驻设备。验收标准明确：**同步次数/步 → O → 单流 tok/s**。
...
与官方二进制性能持平），且剖析已把根因定位到具体机制（每次 decode 的图被切为 2 段，
每步 23 次同步 + 10 次拷贝，见 6.11–6.12）。
...
**根因已定位**（见 6.11–6.12）：每次 decode 的计算图因「嵌入表驻 CPU + logits 输出缓冲在主机」
被切成 2 段，导致每步 23 次流同步 + 10 次拷贝 —— GPU 占空比仅 25%，
单流时间几乎全花在跨后端同步上，而非计算。
...
- ✅ 可重复构建 CUDA 引擎（已达成）
- ✅ 根因量化到具体机制与可验收指标（已达成）
- ⏳ 待决：是否投入图切分/同步消除（天级工作量、需正确性回归守护）
