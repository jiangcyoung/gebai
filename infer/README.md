...
⚠️ 服务端并发聚合吞吐（53 t/s）反而低于单流（143 t/s），甚至低于串行（约 5.4s）——
但**这不是 MoE 的固有特性**：同一模型用 `llama-batched-bench` 直接测批量解码，聚合吞吐几乎线性扩展
（见 6.11），说明批量本身有效，是 **llama-server 的多 slot 路径没吃到批处理红利**。
→ **集成建议：GEBAI 调用本地模型时优先串行**；批量场景改用批处理基准验证过的路径。
