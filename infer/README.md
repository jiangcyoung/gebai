...
用 Bun 原生并发 + **服务端内部计时（`timings`）+ 预热**测得（100 token/请求，np=16）：
...
**642 t/s 已超过统一批处理的理论值**（`llama-batched-bench` B=8 → 391 t/s）——
连续批处理的动态填充优于固定批。
...
> 注意：服务端连续批处理的实测聚合（np=16 并发 16 → **642 t/s**）**超过**同步批处理的
...
`concurrent` 档（np=16）聚合 642.1 t/s（**4.59×**，超过统一批处理 B=8 的 391）——见 6.9。

**为什么不能靠“减少同步次数”拿到大幅提速**（定量依据）：explore 给出的时间预算显示，
23 次同步 + 10 次拷贝合计仅 **0.27 ms/token**（占 7.1 ms 的 3.8%），而
**采样等待 4.6 ms** 与 **graph launch 0.7 ms** 才是主体。
4.6 ms 的本质是 **320 次小 GEMM 的串行依赖气泡**（GPU 占空比仅 25%）——这是单流的
结构性极限，**只能靠批处理摊薄**，而不是靠消除同步。
