...
**最关键的线索（已量化坐实）**：解码期的流同步按秒分布呈断崖式特征：

| 时段 | 同步次数 | 合计耗时 | 对应阶段 |
|---|---|---|---|
| 第 3–9 秒 | 每秒钟 ~100 次 | 合计 ~4 ms/秒 | 模型加载（便宜） |
| **第 10 秒** | **1 158 次** | **354.2 ms** | **解码期** |

```
1 158 次 ÷ 48 token = 24.1 次同步/token
每次平均 306 µs
24.1 × 306 µs = 7.4 ms/token
实测每 token（B=1）：  7.3 ms/token     ← 完美吻合
```

**结论：单流解码的时间几乎全部耗在 ~24 次/token 的流同步上**，而不是 GPU 计算。
这同时解释了此前所有观察：GPU 占空比仅 25%、O ≈ 5.4 ms/步、批量能摊薄（同步一旦发出，
一批内多个序列共享同一次等待）。

**根因推断（待代码级确认）**：同步来自 **qwen35moe 的循环状态机制**。
源码中可见 DeltaNet 层通过 `mctx_cur->get_r_l(il)` 逐层取/存循环状态（`state_predelta` 回调），
而 `llama-memory-recurrent` 的状态管理在 llama.cpp 中常伴随同步。
— 40 层中 30 层是线性注意力（DeltaNet），10 层是 full attention，与“每 token 数十次同步”量级一致。

**优化靶心（具体、局部、可验证）**：设法消除或减少解码期的逐层同步——
而不是“重写整个引擎”。可行手段包括把循环状态操作合并进同一张图、
用事件等待代替阻塞同步、将状态缓冲常驻设备不往返主机。
验收标准明确：同步次数/token 下降 → O 下降 → 单流 tok/s 上升。

**批量时该开销被摊薄**（与“固定开销”的判定一致）：
B=8 时每 token 2.54 ms = 5.4/8 + 1.87 —— 正好对上，说明 O 确实是**每次调用**的固定成本。

> 提醒：nsys 在 WDDM 下的内核计时会低估（本报告显示“GPU 忙碌 7.025 s”里含 6.46 s 模型加载传输；
> “利用率 91.3%”同样被加载期抬高）。看解码期结论要以**内核调用计数与网格配置**为主，
> 以时间为辅——本节的四个结论均建立在计数与配置上。
...
与官方二进制性能持平），且剖析已把靶心收敛到一个具体点（解码期逐层同步）。
...
**靶心已收敛到一点**（见 6.12）：解码期 ~24 次/token 的流同步（每次 306 µs）
占据单流全部时间。修复面小、验收标准明确（同步次数 → O → tok/s）。

风险仍在：llama.cpp 的循环状态（`llama-memory-recurrent`）与图构建耦合较深，
改动需逐项过 `verify.ps1` 质量回归；且上游演进快，补丁需 rebase。

### 启动门槛

- ✅ 已有可重复构建的 CUDA 引擎（已达成）
- ✅ 靶心已量化到具体指标（已达成）
- ⏳ 待决：是否投入逐层同步消除（天级工作量、需正确性回归守护）

> 现状：**140 t/s 是 llama.cpp 现有实现的稳态上限，且已完全用尽参数层空间**。
> 进一步提速必须改引擎代码；不改也能用——多会话并发已能拿到 2.67× 聚合。
