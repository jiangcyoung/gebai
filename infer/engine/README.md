...
### 自建 vs 官方预编译（严格交替 A/B，各 3 轮）

| 引擎 | tg256 均值 | pp512 均值 | batched-bench B=8 | 输出一致性 |
|---|---|---|---|---|
| 官方 win-cuda-12.4 | 145.91 | 4 753 | 508–512 t/s | 基准 |
| 自建 CUDA 12.9 + LTO + native | 147.01 | 4 590 | 496–503 t/s | **逐字节一致** |

**→ 性能基本相当（差异在 ±1% 噪声内）**，输出完全一致。

编译层优化的实测结论：

| 尝试 | 结果 |
|---|---|
| `GGML_LTO=ON` + `GGML_NATIVE=ON` | 与不开 LTO 基本持平（147.01 vs 146.95）——无可测量收益 |
| `CMAKE_CUDA_FLAGS` 加 host 优化标志 | ❌ 与 try-compile 的 Debug flags 冲突（`/RTC1` vs `/O2` → D8016），需用 `_RELEASE` 变体或干脆不加 |
| 首次测到的“自建快 6.8%” | 冷启动偏差，交替 A/B 后不成立（已纠正） |

**因此自建的价值是「可改源码」，而不是速度**：收益来自源码级改动，编译选项已无空间。

> **实测捷径**：上述工具链约束**只影响自建**。官方预编译的 win-cuda-12.4 包不需要任何编译器，
> 且补上 `cudart64_12.dll + cublas64_12.dll + cublasLt64_12.dll` 后**完全自包含**，
> 不依赖 CUDA Toolkit 安装（已验证）。日常部署用官方包即可。
