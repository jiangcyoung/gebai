...
GEBAI 的目标是**融合旧世界IT的所有技术，打造新世界智能的躯体**。它是一个极致动态扩展能力的智能体：单 TS 文件即可定义子 Agent 扩展能力。核心是稳定的「对话 → 工具调用 → 审批 → 执行」主循环，支持多用户安全隔离、多端部署（桌面 WebView / 浏览器 / 服务端）、以及通过修改自身代码实现的代码级自我优化。
