/**
 * 流式聚合基础件（nsight 专用）：超大报告（千万级事件）分析的关键——内存占用与报告规模解耦。
 *
 * 三条纪律：
 * 1. **不物化全量事件**：一律 `iterate()`（SQLite 游标）逐行消费，聚合在流上完成；
 * 2. **结果有界**：排行用 Top-K、分位数用受控采样、时间线用固定桶直方图，输出条数由参数封顶；
 * 3. **一次扫描多次复用**：聚合事实（facts）按报告缓存（报告不可变，缓存按文件指纹失效），
 *    概览/热点/诊断等多次调用共享同一份扫描结果，避免重复全表扫描。
 */
