...
- **JS 执行**：evaluate 用于读取动态数据或模拟复杂交互（如滚动、收集链接），结果自动 JSON 序列化；表达式与函数字面量（`() => …`，会自动调用）均可；不用于绕过页面限制做恶意操作。**页面内 fetch 取 JSON 先验响应类型**：先看 `r.ok`/`content-type`（或先 `r.text()` 再自行 `JSON.parse`），对非 JSON 响应（如 404 的 text/plain）调 `r.json()` 会抛 JSON 解析错误——那描述的是响应体、不是本工具的问题；同理页面内抛错时看错误首行的类型与消息（ReferenceError/TypeError 等均如实回传，不是工具异常）。
...
- **性能 / 动画采样必须先确认页面可见**：rAF 帧间隔、longtask 这类采样只在**可见页**有效——标签页在后台时 rAF 被节流到 ~1fps，采出来是「单帧 800ms+、longtask 0 个」的假数据（很容易被读成「动画被走了合成器」）。多标签时先 close_page 让目标页成为可见页（或切到前台），并把 `document.visibilityState` 一起记进采样结果：采到 hidden 就作废重来。采样脚本里用 `requestAnimationFrame` 自排队计时（而非 setInterval），才能量到真实帧节奏。
- **被委托验证（subsession_run 形态）**：被 code/self_optimize 等子Agent 委托做浏览器端验证时——先明确验证目标与预期结果，再打开页面操作，用 content/screenshot 取证（给出证据位置），最后给出明确结论（符合预期 / 不符合之处），不要只交原始截图。
  - **断言前先等页面就绪，不拿骨架当结果**：open 之后先 `wait_for` 一个**业务标记**（关键容器/`body[data-*]` 属性/首条数据行）再做断言；没有标记就先用 content 确认页面已渲染。骨架窗口（SPA 刚加载、构建产物刚被替换）里看不到任何业务元素，此时断言得到的“什么都没有”是**假的失败**。
  - **目标站点开着热重建/自动刷新时全程盯标记**：构建会先清空产物再写入，页面在构建窗口内只有静态壳、随后自动刷新。开始断言前等标记就绪，并**在结束时重核标记是否还在**（不在了 = 中途被刷新过，之前的断言全部作废、需重跑）。
  - **断言尽量吃结构化数据**：evaluate 的返回值同步以 `data.value` 给出（JSON 解析后的对象/数组），直接用它与期望值比对（`expect`/深比较），不要再去解析 output 文本；content 也给 `data.text`/`data.html`，pages 给 `data.pages`（按 `index`/`active` 选页）。
