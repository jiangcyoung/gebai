 ...
 *
 * 桥接基建（node driver.mjs 子进程 JSON-RPC、playwright 模块/channel 解析、惰性共享单例、
 * 会话串行化）在 core/browser/bridge.ts——平台级底座，playwright/reverse_site 子Agent 与
 * 透明浏览器代理（core/browser/fetch-proxy.ts）共用同一桥接进程与浏览器会话。
 * Windows 默认驱动系统自带 Edge（channel=msedge，免下载浏览器），GEBAI_PLAYWRIGHT_CHANNEL 覆写。
 *
