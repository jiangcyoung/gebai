...
- `torch_findings` **问题诊断清单**（量化证据 + 根因 + 修复方向 + 关联符号，含前向/反向占比），`locate=true` 同时把热点落到源码 `文件:行`；`save=true` 导出 Markdown（`save_dir` 指定目录）
- `torch_compare` **两份 trace 对比**（改前改后：度量差异 + 问题清单增删与净变化；度量按同名对齐、单位不同不比、无固有方向的指标只给变化量）
...
5. **改前改后**：优化前后各一份 trace 时用 `torch_compare`——比人肉读两份输出可靠；单份分析要归档/贴给同事时给 `torch_findings` 传 `save=true`。
