/**
 * shotcraft 测试用 ToolContext：真实文件系统（mkdtemp 临时 home）+ 会话已读追踪桩。
 * 不注入预置项目、不联网；进程 env 中的 SHOTCRAFT_* 由用例显式清理，避免宿主配置影响断言。
 */
...
/** 清理宿主进程可能存在的 shotcraft 环境变量，保证用例不依赖开发机配置。 */
export function clearShotcraftEnv(): void {
  for (const key of Object.keys(process.env)) {
    if (key.startsWith("SHOTCRAFT_")) delete process.env[key]
  }
}
