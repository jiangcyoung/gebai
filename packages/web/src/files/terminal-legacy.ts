/**
 * 文件工作台 · 终端面板（降级实现：管道式持久 shell，非 PTY）。
 *
 * 何时用：服务端探测到 ConPTY 驱动不可用（非 Windows / 无 .NET Framework 编译器）时，
 * 终端门面（`terminal.ts`）落到这里；正常环境由 `terminal-pty.ts`（真 PTY + xterm）接管，
 * 本文件不参与。
 *
 * 命令回显为什么在前端：管道模式没有 tty 回显，所以「提示符 + 命令」由前端在提交瞬间本地回显；
 * 命令结束由服务端注入哨兵行，解析成 `exits[{token,code,cwd}]` 随 `read` 返回，前端据此补一行
 * 退出码（非 0 才提示）并用返回的 cwd 刷新标题栏当前目录。
 *
...
export function createLegacyTerminalPanel(hooks: TerminalHooks): TerminalPanel {
