/**
 * 编辑器右键的**引用文本**：绝对路径 + 行号（工作台「复制绝对路径」与「发送到对话输入框」共用）。
 *
 * 为什么单独成模块：这两处拼的是**给人和给模型看的字符串**，规则必须一致且可测——
 * - 「复制绝对路径」：`/abs/path/src/main.ts:12`（单行）或 `/abs/path/src/main.ts:12-20`（选中块）；
 * - 「发送到对话输入框」：同一套引用 + 代码块（模型据此直接 `read` 该文件的那几行）。
 *
 * 三个容易错、又都不报错的点：
...
/**
 * 组装「发送到对话输入框」的片段：**引用行 + 代码块**（Markdown）。
 *
