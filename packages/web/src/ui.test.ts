...
// 本文件在模块顶层整体替换了 document/window/navigator/location——按基线约定（scripts/test-preload.ts）
// **用完必须放回**：整体替换而不还原会把测试顺序变回变量（后续文件读到本文件的桩，
// 表现为单文件全过、全量随文件执行顺序报错）。
const prevGlobals = {
...
afterAll(() => {
  ...
  // 只在原本存在时才写回：若本文件是在无基线环境（preload 未生效）下首个装桩者，把 document
  // 写回为 undefined 比原来的「不还原」更糟——后续文件会拿到一个存在但为 undefined 的全局。
