/**
 * 子代理契约层（引擎 ↔ 子代理的标准接口，双方向唯一耦合点）。
 * - 引擎实现 ToolContext 注入、消费 SubAgentDef/Tool 定义；
 * - 子代理包只 import 本文件（+ 本包其余契约/工具），零引擎内部依赖——编译期可验证解耦。
 * 类型形状与引擎侧（server core/base/types.ts）保持结构兼容：本文件为契约源，
 * server 侧以 re-export 过渡，最终子代理全部改从 @gebai/sdk 引用。
 */
import type {
  AttachmentRef,
  ContentBlock,
...
/** 引擎交互模式（DESIGN「交互模式」）：none=无交互、multi_turn=多轮交互、realtime=实时交互。
...
export type ToolContext = {
   ...
   * 用户提交后值写入任务 env 返回 true；拒绝/超时返回 false。
   */
