...
const SUMMARIES: Record<string, Record<string, string>> = {
  ...
  "/api/v1/tasks": { get: "任务清单（定时/普通/闲时）", post: "创建任务" },
  "/api/v1/tasks/{id}": { get: "任务详情", patch: "修改任务", delete: "删除任务" },
  "/api/v1/tasks/{id}/run": { post: "手动执行任务（入队）" },
  "/api/v1/tasks/{id}/front": { post: "排队中置顶" },
  "/api/v1/tasks/{id}/queue": { delete: "出队（取消排队）" },
  "/api/v1/tasks/{id}/stop": { post: "终止运行中的任务" },
  "/api/v1/tasks/queue": { get: "任务队列视图（额度/排队/运行中）" },
  "/api/v1/tasks/{id}/files": { get: "任务资源文件清单", delete: "删除任务资源文件" },
  "/api/v1/tasks/{id}/files/content": { get: "读取任务资源文件", put: "写入任务资源文件" },
  "/api/v1/todos": { get: "用户级待办清单", post: "新增待办", patch: "清单级重排" },
  "/api/v1/todos/{id}": { patch: "更新待办", delete: "删除待办" },
  "/api/v1/todos/{id}/run": { post: "立即执行待办（入队执行一次）" },
  ...
  "/api/v1/fs/delete": { post: "删除（物理删除，不可恢复）" },
