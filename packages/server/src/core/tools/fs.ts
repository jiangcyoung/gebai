 ...
 *  正则 test 是**同步 CPU 操作、不可中断**（连 setTimeout 回调也要排在它后面），而真实工作量是
 *  「每文件 × 每行」各调一次（数千次）——故**单次调用的引擎级缓解约束不了整体**：
 *  - 单次就有界但远非小数字：本项目仅部署 Bun（JSC），其回溯预算上界实测随形态而异——
 *    `(a+)+b` ~0.78s、`(a*)*b` **~2.1s**、`(a|a)+b` 在长输入达 **3.5s**（亦有 14ms 级形态，差两个数量级）；
 *    即使单次，2.1s 的同步阻塞对一个多会话服务已不可接受。
 *  - 累积才是决定性的：实测 **20 次调用即 >10s**（强杀）；按 ~0.78s/次外推，1000 行 ≈ 13 分钟、
 *    5000 行 ≈ 66 分钟。
 *  - 该预算属**引擎内部实现**（非文档化契约），把安全性建立在它上面很脆（引擎升级可能改变）。
 *  故隔离到独立子进程 + 超时强杀：把「无界挂死」变成「20s 内有界失败 + 引导」，且期间主进程
 *  事件循环照常服务其他会话（否则全部会话 WS/HTTP 冻结）。 */
