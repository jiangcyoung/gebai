/**
 * 客卿发现器（core/agents/keqing.ts）：扫描 manifest（agent.json）→ 启动
 * 边车进程 → init/tools.list 握手 → 构造标准 SubAgentDef 注册进 SubAgentManager。
 *
 * 发现范围（同名跨语言合并，见 README「跨语言同名合并」）：
 *   1. 内置源 keqing/（仓库根，按实现语言分目录：python/cpp/rust/…——语言目录下共享
 *      基础框架驱动，每个二级目录一个子代理项目；dist 形态产物同构、二进制形态物化
 *      {GEBAI_HOME}/vendor/keqing/——安装包预置物化；构建时过滤 venv/编译产物）
 *   2. 用户自建 {GEBAI_HOME}/agents/{name}/agent.json（放一个目录即成一个子代理，任意语言；
 *      与内置源同名去重时后者胜——manifest 层面的同名覆盖，与 TS+客卿 合并是两回事）
 *
 ...
 * 门控（DESIGN「客卿」）：仅本地形态启用——沙箱启用（服务端部署）或 GEBAI_KEQING=off
 * 时发现器整体禁用（客卿子代理完全不可见），不影响 TS 子代理。
 ...
 *  → 仓库根 keqing/python/venv（语言目录 venv，源码形态）→ PATH（python3/python/py，
 *  Bun.which）。 */
 ...
 *  keqing/python/venv；目录不存在返回 null（dist/二进制形态不用源码树 venv）。 */
function sourceTreeVenv(): string | null {
  const dir = join(import.meta.dirname, "..", "..", "..", "..", "..", "keqing", "python", "venv")
  return existsSync(dir) ? dir : null
}

export interface KeqingManifest {
  name: string
  description: string
  protocol: number
  command: string[]
...
export function parseManifest(raw: string, source: string): { manifest?: KeqingManifest; error?: string } {
...
function pickBuildCommand(build?: KeqingBuild): string[] | null {
 ...
 *  声明 → 跳过；构建失败抛错（launchKeqing 记入 loadErrors，不阻断其他子代理）。 */
export async function ensureBuilt(
  dir: string,
  manifest: KeqingManifest,
...
export interface KeqingRunnerOptions {
  /** 测试注入：spawn 替身与解释器解析替身。 */
  spawn?: SidecarSpawnFn
  resolvePython?: () => string[] | null
  /** 跳过边车启动握手（单测：只验证 manifest 解析与 def 构造）。 */
  skipHandshake?: boolean
  /** 覆盖发现根目录（测试隔离；缺省 keqingRoots()——内置 + {GEBAI_HOME}/agents）。 */
  roots?: string[]
  env?: Record<string, string>
}

/** 启动一个 客卿子代理并握手，返回 SubAgentDef 与边车实例（失败抛错，调用方记 loadErrors）。
 *  边车实例登记进模块级 liveSidecars（进程级共享：签名命中缓存复用时新实例不重启进程；
 *  热加载重扫时同名先 dispose 再拉新、消失的名单由 discoverKeqing 对账回收）。 */
export async function launchKeqing(
  dir: string,
  manifest: KeqingManifest,
  opts: KeqingRunnerOptions = {},
...
/** 客卿源目录们（keqing roots）（供发现器与目录签名共用）：内置源按实现语言展开（每个语言目录一个
 *  扫描根——语言目录下二级目录即子代理项目）+ 用户自建 {GEBAI_HOME}/agents。 */
export function keqingRoots(): string[] {
  const roots: string[] = []
  const src = keqingSourceDir()
  try {
...
/** 发现并启动全部 客卿子代理（发现器入口）：扫描内置 + 用户自建 manifest，逐个启动握手，
 *  返回成功 defs 与失败清单（name → 错误）；单项失败不阻断其他项（同名去重：用户自建覆盖内置）。 */
export async function discoverKeqing(
  opts: KeqingRunnerOptions = {},
): Promise<{ defs: SubAgentDef[]; errors: Array<[string, string]> }> {
  const roots = opts.roots ?? keqingRoots()
  ...
  await Promise.all(
    [...byName.entries()].map(async ([name, f]) => {
      ...
      try {
        const { def } = await launchKeqing(f.dir, m.manifest, opts)
        defs.push(def)
      } catch (err) {
...
export function disposeKeqingNotIn(names: string[]): void {
...
/** 全部回收（服务关闭/测试收尾用）。 */
export function disposeAllKeqing(): void {
  for (const [name, sc] of [...liveSidecars]) {
    sc.dispose("服务关闭")
    liveSidecars.delete(name)
  }
