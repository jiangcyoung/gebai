...
//
// 为什么原生：事件库可达 GB 级、千万级事件，宿主 JS 层逐行取列与聚合的开销是主成本
// （实测 1000 万行：JS 宽行扫描 8.7s + SQL 分组 9.3s ≈ 18s；本实现单趟同等工作量约 4s）。
...
// 会变成主成本（实测：逐行构造 4 个 BTreeMap 使 10M 行的聚合从 4s 级别恶化到 20s 级别）。
