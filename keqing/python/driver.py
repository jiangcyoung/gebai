#!/usr/bin/env python3
"""Python 语言基础框架驱动（歌白客卿协议 v1，见 keqing/README.md）。

语言目录 keqing/python/ 下共享：本驱动 + venv + requirements.txt；每个子代理项目
...
TOOLS = [
    ...
    {
        "name": "pip",
        "description": "Python 依赖管理（venv 与 requirements.txt 位于 Python 语言目录 keqing/python/）。action=install 安装（packages 指定包名列表/空格分隔字符串；缺省则 -r requirements.txt；venv 不存在自动创建；装完边车自动重启加载新依赖）action=freeze 快照当前依赖写回 requirements.txt；action=status 查看 venv/已装包。",
        "parameters": {
            "type": "object",
            "properties": {
