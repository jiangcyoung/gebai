...
| `Ctrl+\` | 开关文件工作台：会话桌面 → **并列**（文件工作台停靠一侧，可在工作台「更多」菜单里换侧）→ 回会话桌面；窗口容不下并列时它是**整窗**开关，整窗态下它回会话工作台——连按两次总能回到会话桌面 · 工作台里那一侧的 `Ctrl+\` 是「回到会话工作台」（并列 / 整窗都适用） |
...
| `Ctrl+K` | 「更多」菜单（新建 / 比较 / 重新加载 / 快捷键 / 服务端开关 / 全屏 / 停靠侧 / 回到会话工作台） | 接管「地址栏搜索」 |
| `Ctrl+\` | 回到会话工作台（关闭并列 / 退出整窗）——嵌入态（分屏 iframe）才有的键 | 与宿主主界面的 `Ctrl+\` 同一个键：那边管「开关」，面板里这个管「退出」 |
| ``Ctrl+` `` | 底部终端工具窗 | — |
